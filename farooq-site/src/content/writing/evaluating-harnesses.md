...
Most teams choose between agent harnesses by running a benchmark and picking the higher score: one scores 76%, the other 74%, done. That method is broken. The two papers that formalized harness engineering in March 2026 show why: across harness configurations, resolve rates barely move, while cost moves by an order of magnitude. In this post I want to give you a method that works instead. I'll walk through the five steps I would run on any harness, the benchmarks to run them on, and where those benchmarks fall short, with a checklist at the end. If you're new to this topic, the background lives in two earlier essays: [what a harness is](/writing/what-is-an-agent-harness/) and [the evidence that the harness now matters more than the model](/writing/harness-engineering/).
...
When accuracy saturates, cost becomes the differentiator. So the first rule of harness evaluation is to never report a resolve rate without the cost next to it. Cost is not one mysterious thing, it is three plain numbers you can log:

- prompt tokens per sample
- tool calls per sample
- wall-clock time per sample

Those four numbers per configuration, resolve rate plus the three cost numbers, are the raw material for the method below.

## The five steps

**1. Fix the model, vary the harness.** A comparison only means something as a controlled experiment. Harness comparisons used to be impossible because systems differed in everything at once: prompts, tools, verification gates, and state semantics, all tangled together. The NLAH design untangles them. It separates an agent into a backend (the tools and infrastructure), a runtime charter (the universal execution rules), and the harness itself (the task-specific control logic). Hold two layers fixed, swap the third, and a difference in the numbers means something. The payoff is real. When the Tsinghua team migrated OS-Symphony, a desktop automation harness, from native code into the NLAH representation with the same model and the same strategy, accuracy jumped from 30.4% to 47.2%, runtime fell from 361 minutes to 141, and LLM calls collapsed from 1,200 to 34. Without the separation, nobody could have attributed that gain to the representation.

**2. Ablate every module, and expect surprises.** To find out which modules earn their keep, turn each one off, re-run, and record the delta on both accuracy and cost. The NLAH ablations show that intuition fails here:
...
The shared limit: none of them reports cost as a first-class metric. Resolve rate is the leaderboard; tokens, calls, and minutes you have to instrument yourself. Until that changes, public rankings will keep calling a 16-million-token harness and a 1-million-token harness equivalent. The method above is something you have to run, not something you can look up.
