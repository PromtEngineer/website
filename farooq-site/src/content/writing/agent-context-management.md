...
Context management is how an agent harness decides what the model sees on each turn. It matters because the context window is too small for everything the model might want to see, and a full window makes the agent worse. You can watch this happen: an agent works well for twenty minutes, reads one big file, and gets noticeably dumber. It repeats itself, forgets a decision it made earlier, and re-reads things it already saw. Nothing about the model changed. The window filled up, and the model stopped reasoning about the task and started swimming through its own transcript.

Modern harnesses treat context as memory that has to be managed, not as a passive chat log. In [the harness anatomy essay](/writing/what-is-an-agent-harness/) I listed context management as component two of nine. In this post I want to go deeper: what occupies the window, the four moves harnesses make when content doesn't fit, and how four real harnesses handle the same stress test.

## What fills the window

The window holds more than the conversation. On any given turn it contains:
...
Almost everything on that list grows, and the window doesn't. All of it competes with the one budget you can't touch: room for the model to answer.
...
The harness ends up acting like an operating system. It decides what stays close to the model, what gets compressed, what gets paged out, and what can be retrieved later.

## How harnesses handle large files

The clearest test case is a file that's bigger than the available context. When the model asks to read one, somebody has to decide what happens, because "just read it" would evict everything else the model knows. For the research behind this post I traced that decision through four harnesses: Pi, OpenClaw, Claude Code, and Letta. None of them just reads the file. The answer is always some combination of four moves:
...
Each harness mixes the moves differently:
...
Different architectures, same lesson: file context isn't dumped into the model, it's mediated.

## Tool results are the quiet offender

Tool results eat context in a way that's easy to miss. A file announces itself: you know a 200 KB read is a problem the moment you see it. Tool results feel harmless because each one looks useful in isolation. One grep output is fine. One JSON payload is fine. But every call writes another object into the transcript, so a session's worth of grep outputs, logs, and dataframe previews can eat the working set faster than the conversation itself, and no single result ever looked like the culprit.
...
The insight underneath is that the model doesn't need the entire payload in the prompt. It needs enough visible state to decide what to fetch next.

## Old history gets compacted
...
Bigger windows don't dissolve the problem either. [Lost in the Middle](https://arxiv.org/abs/2307.03172) showed that models use information at the start and end of a long context far better than information buried in the middle. Chroma's [Context Rot](https://www.trychroma.com/research/context-rot) report measured 18 models getting less reliable as input grows, even on simple tasks. Putting more tokens in the window doesn't guarantee the model uses them well, so the management problem stays.
...
The agent that got dumber after one big file read didn't have a model problem. It had a paging problem. The agent looks like one continuous intelligence, and underneath, the harness is constantly moving memory around to keep that illusion alive.
